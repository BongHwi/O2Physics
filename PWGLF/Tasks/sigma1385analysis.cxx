...
struct sigma1385analysis {
  ...
  Partition<aod::ResoMCParents> selectedMCParticles = (aod::mcparticle::pdgCode == static_cast<Int_t>(313)); // K(892)0
  ...
  PROCESS_SWITCH(sigma1385analysis, processData, "Process Event for data", true);
                 ...
                 soa::Join<aod::ResoTracks, aod::ResoMCTracks> const& resotracks, aod::ResoMCParents const& resoParents, aod::McParticles const& mcParticles)
  {
    ...
    for (auto& collision : collisions) {
      ...
      for (auto& part : particles) {             // loop over all MC particles
        ...
        if ((abs(part.daughterPDG1()) == kKPlus && abs(part.daughterPDG2()) == kPiPlus) || (abs(part.daughterPDG1()) == kPiPlus && abs(part.daughterPDG2()) == kKPlus)) {
          histos.fill(HIST("truek892pt"), part.pt());
        }
      }
    }
  }
  PROCESS_SWITCH(sigma1385analysis, processMC, "Process Event for MC", false);
  ...
  PROCESS_SWITCH(sigma1385analysis, processME, "Process EventMixing", false);
};

WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  return WorkflowSpec{adaptAnalysisTask<sigma1385analysis>(cfgc, TaskName{"lf-sigma1385analysis"})};
}
